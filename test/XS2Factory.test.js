...
var XS2Factory = artifacts.require("./XS2Factory.sol");
...
contract("XS2Factory", (accounts) => {
    ...
    before(async () => {
        ...
        option_master = await XS2Option.new();

        factory = await XS2Factory.new(option_master.address);

        const block = await web3.eth.getBlock("latest");
        let option_tx = await factory.deploy("XS2/USDT 0.02 1hr", "XS2USDT0.02", xs2.address, usdt.address, "20000", block.timestamp + 3600);
        option1 = await XS2Option.at(option_tx.receipt.logs[0].args[0]);

        option_tx = await factory.deploy("XS2/USDT 0.04 1hr", "XS2USDT0.04", xs2.address, usdt.address, "40000", block.timestamp + 3600);
